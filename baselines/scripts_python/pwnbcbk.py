import pandas as pd
from baselines.scripts_python.python_packages.pwNBCBk.pwnbcbk import pwNBCBk
# from baselines.scripts_python.python_packages.CITMI.citmi_simple import PCTMI


def pwnbcbk(data, sig_level=0.05, nlags=5, verbose=True, pairwise=True):
    citmi = pwNBCBk(data, sig_lev=sig_level, lag_max=nlags, p_value=True, rank_using_p_value=False, verbose=verbose,
...
if __name__ == "__main__":
    ...
    data = data.loc[:1000]
    print(data)
    df = nbcb(data, sig_level=0.05, nlags=5, verbose=True)
    print(df)
