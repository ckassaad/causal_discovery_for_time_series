...
def distance_sklearn_metrics(z, k = 4, metric = 'cosine'):
    """Compute exact pairwise distances."""
    d = sklearn.metrics.pairwise.pairwise_distances(
            z, metric = 'correlation',  n_jobs=1)
    # k-NN graph.
    idx = np.argsort(d)[:, 1:k+1]
    d.sort()
    d = d[:, 1:k+1]
...
def adjacency_randomwalk(dist, idx, locations, sigma = None):
    ...
    for i in range(M):
        for j in range(k):
            ...
            #Lx[t][i] = sign(xdist) * math.exp( -xdist ** 2 / sigma)/k;
...
#    L_idx = torch.zeros(M, k).long();
    ...
    for i in range(M):
        for j in range(k):
            ...
            xdist = (locations[t][0] - locations[i][0])
            ydist = (locations[t][1] - locations[i][1])
            xexp = math.exp( - (xdist ** 2)/ sigma)
            yexp = math.exp( - (ydist ** 2)/ sigma)
            dist = math.exp( - (xdist ** 2 + ydist ** 2)/ sigma)
            
            L[i][j][0] = (xexp);
            L[i][j][1] = (yexp);
            L[i][j][2] = dist
            L[i][j][3] = np.sign(xdist);
            L[i][j][4] = np.sign(ydist);
            #L[i][j][3] = locations[i][0]/max_0; L[i][j][4] = locations[i][1]/max_1;
            L_idx[i][j] = float(i * M + t);
...
def adjacency_xy(dist, idx, locations, sigma = None):
    M, k = idx.shape;
    if sigma is None:
        sigma = np.mean(dist[:, -1])**2
    print('sigma:', sigma);
    ...
    for i in range(M):
        for j in range(k):
            ...
            dist = xdist ** 2 + ydist ** 2
            #Lx[t][i] = sign(xdist) * math.exp( -xdist ** 2 / sigma)/k;
            Lx[t][i] = math.exp( -xdist ** 2 / sigma);
            Ly[t][i] = math.exp( -ydist ** 2 / sigma);
...
def adjacency_xy4(dist, idx, locations, sigma = None):
    M, k = idx.shape;
    if sigma is None:
        sigma = np.mean(dist[:, -1])**2
    print('sigma:', sigma);
    ...
    for i in range(M):
        for j in range(k):
            ...
            if (xdist > 0):
                Lxp[t][i] = math.exp( -xdist ** 2 / sigma) / k;
            else:
                Lxn[t][i] = math.exp( -xdist ** 2 / sigma) / k;
            if (ydist > 0):
                Lyp[t][i] = math.exp( -ydist ** 2 / sigma) / k;
            else:
                Lyn[t][i] = math.exp( -ydist ** 2 / sigma) / k;
    return Lxp, Lxn, Lyp, Lyn;
