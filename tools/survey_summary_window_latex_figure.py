if __name__ == "__main__":
    ...
    path_output = "../experiments/performance_average/latex_format/figures" + "/" + measure_name + \
                  "_survey_summary_window_latex_figure_.txt"
    ...
    for structure in structure_list:
        measure = ""
        for type_of_causes in ["summary_other_and_self", "summary_other_and_self", "temporal"]:
            ...
            if type_of_causes == "summary_other_and_self":
                ...
                all_methods_list = ['TCDF', 'PCMCICMIknn', 'PCMCIParCorr', 'tsFCI', 'VarLiNGAM',  'Dynotears']
                methods_treats_hidden = ['TCDF', 'tsFCI']
            ...
            if structure == structure_list[0]:
                if type_of_causes == "summary_other_and_self":
                    if "Oriented" in measure:
                        title = "\n\t\ttitle = {Summary graph: arrows},"
                    else:
                        title = "\n\t\ttitle = {Summary graph: edges},"
                else:
                    title = "\n\t\ttitle = {Window causal graph},"
            else:
                title = ""
            if (type_of_causes == "summary_other_and_self") and (measure == measure_name + " " + "Adjacent"):
